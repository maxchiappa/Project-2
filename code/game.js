...
function Ball(pos, ch) {
  this.pos = pos;
  if (ch == "b") {
	 this.type = "ball";
	 this.size = new Vector(.3, .3);
     this.speed = new Vector(5, -3.2);

  }
  else if (ch == "3") {
	 this.type = "diagonalball";
	 this.size = new Vector(.3, .3);
     this.speed = new Vector(3.7, 3.2);
...
Ball.prototype.act = function(step, level) {
  var newPos = this.pos.plus(this.speed.times(step));
  if (!level.obstacleAt(newPos, this.size))
    this.pos = newPos;
  else if (this.repeatPos)
...
Level.prototype.playerTouched = function(type, actor) {
  ...
  if (type == "lava" && this.status == null ||
   type == "lava" && this.status == "speedboost" ||
      type == "lava" && this.status == "jumpboost" ||
    type == "arrow" && this.status == null ||
    type == "arrow" && this.status == "speedboost" ||
     type == "arrow" && this.status == "jumpboost" ||
    type == "straight" && this.status == null||
    type == "straight" && this.status == "speedboost" ||
       type == "straight" && this.status == "jumpboost" ||
    type == "ball" && this.status == null ||
  type == "ball" && this.status == "speedboost"||
     type == "ball" && this.status == "jumpboost" ||
  type == "diagonalball" && this.status == null||
  type == "diagonalball" && this.status == "speedboost"||
     type == "diagonalball" && this.status == "jumpboost" ||
 type == "otherball" && this.status == null||
    type == "otherball" && this.status == "jumpboost" ||
type == "otherball" && this.status == "speedboost")
 {
    this.status = "lost";
    this.finishDelay = 1;
  } else if (type == "coin") {
